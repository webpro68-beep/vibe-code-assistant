...
export function getSceneIssues(
  ...
  return issues.filter(
    (issue) => issue.target_type === "scene" && issue.target_index === rowIndex,
  );
}

export function getSubtitleIssues(
  issues: ValidationIssue[],
  rowIndex: number,
): ValidationIssue[] {
  return issues.filter(
    (issue) => issue.target_type === "subtitle" && issue.target_index === rowIndex,
  );
}
...
export function getPreviewIssues(
  ...
  return issues.filter((issue) => issue.target_type === "preview");
}
